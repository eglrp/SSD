...
def model(sess):
    ...
    h = [512, 1024, 1024, 256, 512, 128, 256, 128, 256]
    ...
    sess.run(tf.variables_initializer(tf.get_collection(tf.GraphKeys.VARIABLES, scope="ssd_extension")))
    ...
    for i, out in zip(range(len(outputs)), outputs):
        ...
        outfs.append(tf.reshape(out, [-1, w * h * layer_boxes[i], c_ + 4]))
...
# loc loss
def smooth_l1(x):
    return tf.where(tf.less(tf.abs(x), 1.0), 0.5 * (x**2.0), tf.abs(x) - 0.5)


# loss  对应论文中 Training objective 章节
def loss(pred_labels, pred_locs, total_boxes):
    ...
    pos_add_negs = positives + negatives

    class_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred_labels, labels=true_labels) * pos_add_negs
    class_loss = tf.reduce_sum(class_loss, reduction_indices=1) / (1e-5 + tf.reduce_sum(pos_add_negs, reduction_indices=1))
...
# 对应论文的 Choosing scales and aspect ratios for default boxes
def box_scale(k):
    s_min = box_s_min
    s_max = 0.95
    m = 6.0
