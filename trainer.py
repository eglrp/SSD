...
def start_train():
    ...
    while True:
        ...
        _, c_loss_f, l_loss_f, loss_f, step = ssd.sess.run(
            [ssd.optimizer, ssd.class_loss, ssd.loc_loss, ssd.total_loss, ssd.global_step],
...
def show_webcam(address):
    ...
    while True:
        ...
        pred_labels_f, pred_locs_f = ssd.sess.run([ssd.pred_labels, ssd.pred_locs],
                                                  feed_dict={ssd.imgs_ph: [resized_img], ssd.bn: True})
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string("image_path", "input/2.jpg", "path to image")
