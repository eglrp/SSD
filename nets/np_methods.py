...
# =========================================================================== #
def ssd_bboxes_decode(feat_localizations, anchor_bboxes, prior_scaling=list([0.1, 0.1, 0.2, 0.2])):
    """
    ...
    return np.reshape(bboxes, l_shape)


# 将符合条件（非背景得分大于select_threshold）的类别、得分和边界框筛选出
def ssd_bboxes_select_layer(predictions_layer, localizations_layer, anchors_layer,
    ...
    localizations_layer = np.reshape(localizations_layer, (batch_size, -1, localizations_layer.shape[-1]))
...
# 将符合条件（非背景得分大于select_threshold）的类别、得分和边界框筛选出
def ssd_bboxes_select(predictions_net, localizations_net, anchors_net, select_threshold=0.5,
                      img_shape=(300, 300), num_classes=21, decode=True):
    """
    ...
    for i in range(len(predictions_net)):
        classes, scores, bboxes = ssd_bboxes_select_layer(predictions_net[i], localizations_net[i], anchors_net[i],
...
def bboxes_resize(bbox_ref, bboxes):
    """
    Resize bounding boxes based on a reference bounding box, assuming that the latter is [0, 0, 1, 1] after transform.
    """
    bboxes = np.copy(bboxes)
    # Translate.
    bboxes[:, 0] -= bbox_ref[0]
...
def bboxes_jaccard(bboxes1, bboxes2):
    ...
    # 重叠率：交/并
    return int_vol / (vol1 + vol2 - int_vol)
...
# 非极大值抑制(non maximum suppression)：来抑制那些冗余的框
# 抑制的过程是一个迭代-遍历-消除的过程。
